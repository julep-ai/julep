---
title: "CMA Runtime"
description: "Claude Managed Agents as a runtime for the app node."
---
...
references [the Specification](/docs/internals/specification), [Concepts](/docs/concepts/model), and
[Temporal](/docs/deploy/temporal).
...
The north star ([SPEC §1](/docs/internals/specification#1-north-star)) is a *frozen, typed,
...
opaque — the **Sub one-way mirror** ([SPEC §2 invariant 5](/docs/internals/specification#2-invariants-normative)):
...
limits: approval gating is *tool-call*-specific ([SPEC §7.3](/docs/internals/specification#73-approval-gating-new--required)),
...
([SPEC §10](/docs/internals/specification#10-agent-loop)). It is `Shape.AGENT` — the **top** of the
...
`app` contract ([SPEC §10](/docs/internals/specification#10-agent-loop)) also requires budget
...
1. **Tool surface = the frozen manifest, nothing more.** Define the granted
   ...
   **not** include CMA's built-in `agent_toolset_20260401`, or set its
   ...
   ([SPEC §7](/docs/internals/specification#7-capabilities-deny-by-default)). MCP-server grants in
...
3. **Preserve the run_agent boundary and terminal shape.** Carry an `AgentState`
   ...
   "history is the source of truth" invariant ([SPEC §2 invariant 6](/docs/internals/specification#2-invariants-normative)),
...
- **Per-tool-call idempotency keys — addressed.** Each call gets a deterministic
  ...
  ([SPEC §8.5](/docs/internals/specification#85-idempotency-keys)). (There is no documented
...
- **Cancellation — addressed; race admission — inherited.** The driver always
  ...
  effects ([SPEC §8.3](/docs/internals/specification#83-racehedgequorum)). The existing IR-level race
...
- **Human gates — partial.** Approval-required tools still force the loop to stop
  ...
  ([SPEC §7.3](/docs/internals/specification#73-approval-gating-new--required)). Mapping a durable
  `human_gate(...)` signal/wait ([SPEC §8.6](/docs/internals/specification#86-human-gate)) onto CMA's
...
- **Observability / projection — addressed for parity.** The driver returns a
  ...
  per [SPEC §10.1](/docs/internals/specification#101-trace-richness) (and a CMA-session-id projection
