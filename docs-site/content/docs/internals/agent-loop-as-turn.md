---
title: "Agent Loop as Turn"
description: "Modeling the agent loop as a turn-structured trace."
---
...
it is** (see *Why not unify the two loops*). It references [the Specification](/docs/internals/specification)
(§10 agent loop, §4.1 shape lattice, §6 freeze/replay), [the Typed Flow Calculus](/docs/internals/typed-flow-calculus)
(the formal model), and [Cma Runtime](/docs/internals/cma-runtime) (the third `run_agent`
...
- **No new IR, no new recursion primitive** (`algebra.hs` law 6). `drive`/`Step`
  ...
  emits no IR, like the CMA backend ([Cma Runtime](/docs/internals/cma-runtime)).
...
- **The lattice is not collapsed.** Feedback stays strictly below Agent
  ([SPEC §4.1](/docs/internals/specification#41-shape-lattice)); we add no shared node.
