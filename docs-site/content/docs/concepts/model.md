...
That sentence is the framework's contract ([SPEC §1](/docs/internals/specification#1-north-star)).
...
The runtime has three planes ([SPEC §3](/docs/internals/specification#3-architecture--three-planes)):
...
([SPEC §11](/docs/internals/specification#11-projection)): it is useful for value stores, cost by
...
([SPEC §2 invariant 6](/docs/internals/specification#2-invariants-normative)).
...
fans its body out over a runtime list ([SPEC §8.12](/docs/internals/specification#812-each-dynamic-fan-out)).
...
([SPEC §6.3](/docs/internals/specification#63-canonical-json-is-strict)); the deployment artifact hash
identifies the intended program ([SPEC §6.2](/docs/internals/specification#62-artifact-hash)).
...
[SPEC §13](/docs/internals/specification#13-golden-corpus-cross-language-contract) and
[`tests/golden/`](https://github.com/julep-ai/julep-v2/blob/main/tests/golden/).
...
than its children or its operator floor ([SPEC §4.1](/docs/internals/specification#41-shape-lattice)).
...
| `Staged` | `stage` / `eval_plan` | Runtime structure is model-produced and admitted before execution; the plan may be rich up to `Feedback` but may not stage another plan or open an agent loop ([SPEC §9](/docs/internals/specification#9-staged-plans)). |
| `Agent` | `app` | The top of the lattice: an open-ended controller loop with a closed action vocabulary and budget guard ([SPEC §10](/docs/internals/specification#10-agent-loop)). |

There are two reads of the same tree ([SPEC §4.2](/docs/internals/specification#42-surface-vs-closed-shape)).
...
([SPEC §2 invariant 5](/docs/internals/specification#2-invariants-normative)). A sub-flow's value can
...
Deploy is a fixed sequence of gates ([README](/docs#the-compile-pipeline-deploy),
[SPEC §5](/docs/internals/specification#5-compile-pipeline)). The deeper safety discussion belongs in
...
([SPEC §8.2](/docs/internals/specification#82-whole_session-degradation)).
...
deny all ([SPEC §7.1](/docs/internals/specification#71-absent-vs-empty)). A granted tool can also
assert `(effect, idempotency)`, which is the trusted contract surface used by
freeze and race admission ([SPEC §7](/docs/internals/specification#7-capabilities-deny-by-default)).
...
is success-based and cancels losers ([SPEC §8.3](/docs/internals/specification#83-race--hedge--quorum)).
...
([SPEC §2 invariants 2-3](/docs/internals/specification#2-invariants-normative)).
...
fork between "flow mode" and "agent mode" ([SPEC §10](/docs/internals/specification#10-agent-loop)).
...
and race-admitted before it runs ([SPEC §9](/docs/internals/specification#9-staged-plans)). The
...
IR contract. See [the Typed Flow Calculus](/docs/internals/typed-flow-calculus).
...
[Temporal](/docs/deploy/temporal) for deployment, [First Agent](/docs/start/first-agent)
for first use, [Examples](/docs/guides/examples) for worked flows,
[Documentation](/docs) for the docs index, [the Typed Flow Calculus](/docs/internals/typed-flow-calculus) for
