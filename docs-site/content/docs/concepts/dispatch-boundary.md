...
description: "What belongs in a flow versus the dispatch layer."
---
...
| Debounce / batching windows | Collapses *inputs* before any flow exists | `execution.debounce.submit_debounced` (Temporal signal-with-start collator), `dbos.Debouncer` |
...
`ExecutionPolicy.max_parallel`), per-item fan-out over a list the flow already
holds (`each`), branching (`alt`), bounded loops (`iter_up_to`), durable waits
(`delay`, `human_gate`), staged plans, sub-flows, budgets, and the capability
surface the flow runs against (grants live in the capability manifest frozen
alongside the flow).

The debounce collator and `each` are two halves of one batching story: the
collator decides *when a batch exists* (dispatch — collapsing submissions into
one run whose input is the collected list), and `each` decides *how the flow
processes a list it was given* (IR). Neither does the other's job.
