...
`composable_agents.dag`, and lowers to the same `Node` IR emitted by the raw
combinator kernel. The typed layer in `composable_agents.typed` is an
...
`composable_agents.typed`; typed adapters in `composable_agents.flow_adapters`.
...
[CLI docs](/docs/guides/using-the-cli) for `ca`.
