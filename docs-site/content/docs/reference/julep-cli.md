...
| `env.<name>.cas` | `None` | Deploy CAS; implicit `local` defaults to `.julep/cas`. |
...
2. `julep deploy <selector> --env <name>` freezes live source, publishes a signed bundle to CAS, and upserts `.julep/deploys/<name>.json`.
