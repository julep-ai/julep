...
def test_route_create_agent_with_project(make_request, test_project):
    ...
    data = {'name': 'test agent with project', 'about': 'test agent about', 'model': 'gpt-4o-mini', 'project': test_project.canonical_name}
    response = make_request(method='POST', url='/agents', json=data)
    assert response.status_code == 201
...
def test_route_create_agent_with_instructions(make_request):
    ...
    data = {'name': 'test agent', 'about': 'test agent about', 'model': 'gpt-4o-mini', 'instructions': ['test instruction']}
    response = make_request(method='POST', url='/agents', json=data)
    assert response.status_code == 201
...
def test_route_create_or_update_agent_with_project(make_request, test_project):
    ...
    agent_id = str(uuid7())
    data = {'name': 'test agent with project', 'about': 'test agent about', 'model': 'gpt-4o-mini', 'instructions': ['test instruction'], 'project': test_project.canonical_name}
    response = make_request(method='POST', url=f'/agents/{agent_id}', json=data)
    assert response.status_code == 201
    assert response.json()['project'] == test_project.canonical_name
...
def test_route_update_agent(make_request, test_agent):
    """route: update agent"""
    data = {'name': 'updated agent', 'about': 'updated agent about', 'default_settings': {'temperature': 1.0}, 'model': 'gpt-4o-mini', 'metadata': {'hello': 'world'}}
    agent_id = str(test_agent.id)
...
def test_route_update_agent_with_project(make_request, test_agent, test_project):
    ...
    data = {'name': 'updated agent with project', 'about': 'updated agent about', 'default_settings': {'temperature': 1.0}, 'model': 'gpt-4o-mini', 'metadata': {'hello': 'world'}, 'project': test_project.canonical_name}
    agent_id = str(test_agent.id)
    response = make_request(method='PUT', url=f'/agents/{agent_id}', json=data)
    assert response.status_code == 200
    agent_id = response.json()['id']
...
def test_route_patch_agent(make_request, test_agent):
    """route: patch agent"""
    agent_id = str(test_agent.id)
    data = {'name': 'patched agent', 'about': 'patched agent about', 'default_settings': {'temperature': 1.0}, 'metadata': {'hello': 'world'}}
...
def test_route_patch_agent_with_project(make_request, test_agent, test_project):
    ...
    agent_id = str(test_agent.id)
    data = {'name': 'patched agent with project', 'about': 'patched agent about', 'default_settings': {'temperature': 1.0}, 'metadata': {'hello': 'world'}, 'project': test_project.canonical_name}
    response = make_request(method='PATCH', url=f'/agents/{agent_id}', json=data)
    assert response.status_code == 200
    agent_id = response.json()['id']
    response = make_request(method='GET', url=f'/agents/{agent_id}')
...
def test_route_list_agents_with_project_filter(make_request, test_project):
    ...
    data = {'name': 'test agent for project filter', 'about': 'test agent about', 'model': 'gpt-4o-mini', 'project': test_project.canonical_name}
    make_request(method='POST', url='/agents', json=data)
    response = make_request(method='GET', url='/agents', params={'project': test_project.canonical_name})
    assert response.status_code == 200
    response = response.json()
    agents = response['items']
    assert isinstance(agents, list)
...
def test_route_list_agents_with_metadata_filter(make_request):
    ...
    response = make_request(method='GET', url='/agents', params={'metadata_filter': {'test': 'test'}})
    assert response.status_code == 200
    response = response.json()
    agents = response['items']
    assert isinstance(agents, list)
