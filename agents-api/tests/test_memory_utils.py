...
def test_total_size_containers():
    ...
    d = {'a': 1, 'b': 2, 'c': 3}
    expected_min = sys.getsizeof(d) + sum((sys.getsizeof(k) + sys.getsizeof(v) for k, v in d.items()))
...
def test_total_size_nested():
    ...
    complex_obj: dict[str, list[tuple[int, set[int]]]] = {'data': [(1, {2, 3}), (4, {5, 6})], 'meta': [(7, {8, 9}), (10, {11, 12})]}
...
def test_total_size_custom_objects():
    ...
    assert total_size(person, handlers={Person: person_handler}) > total_size(empty_person, handlers={Person: person_handler})
