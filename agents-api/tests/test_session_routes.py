...
def test_route_create_session(make_request, test_agent):
    ...
    data = {'agent': str(test_agent.id), 'situation': 'test session about', 'metadata': {'test': 'test'}, 'system_template': 'test system template'}
...
def test_route_list_sessions_with_metadata_filter(make_request, test_session):
    ...
    response = make_request(method='GET', url='/sessions', params={'metadata_filter': {'test': 'test'}})
    assert response.status_code == 200
    response = response.json()
    sessions = response['items']
    assert isinstance(sessions, list)
