...
def test_middleware_inactive_paid_user_receives_forbidden_response(client):
    ...
    async def test_inactive_paid_user():
        return {'status': 'success'}
    developer_id = str(uuid.uuid4())
    mock_user_cost_data = {'active': False, 'cost': 0.0, 'developer_id': developer_id, 'tags': ['paid']}
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.get('/test-inactive-paid-user', headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert 'Invalid user account' in response.text
        assert 'invalid_user_account' in response.text
...
def test_middleware_cost_limit_exceeded_all_requests_blocked_except_get(client):
    ...
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        post_response = client.post('/test-cost-limit/post', json={'message': 'test'}, headers={'X-Developer-Id': developer_id})
        ...
        put_response = client.put('/test-methods/put', json={'message': 'test update'}, headers={'X-Developer-Id': developer_id})
        ...
        delete_response = client.delete('/test-methods/delete', headers={'X-Developer-Id': developer_id})
        assert delete_response.status_code == status.HTTP_403_FORBIDDEN
        assert 'Cost limit exceeded' in delete_response.text
        assert 'cost_limit_exceeded' in delete_response.text
        get_response = client.get('/test-cost-limit/get', headers={'X-Developer-Id': developer_id})
...
def test_middleware_paid_tag_bypasses_cost_limit_check(client):
    ...
    async def test_paid_methods_delete():
        return {'status': 'success', 'method': 'DELETE'}
    developer_id = str(uuid.uuid4())
    mock_user_cost_data = {'active': True, 'cost': float(free_tier_cost_limit) + 10.0, 'developer_id': developer_id, 'tags': ['test', 'paid', 'other-tag']}
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.post('/test-paid/post', json={'message': 'test'}, headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_200_OK
        assert response.json()['method'] == 'POST'
        assert response.json()['message'] == 'test'
        put_response = client.put('/test-paid-methods/put', json={'message': 'test update'}, headers={'X-Developer-Id': developer_id})
        assert put_response.status_code == status.HTTP_200_OK
        assert put_response.json()['method'] == 'PUT'
        delete_response = client.delete('/test-paid-methods/delete', headers={'X-Developer-Id': developer_id})
...
def test_middleware_get_request_with_cost_limit_exceeded_passes_through(client):
    ...
    mock_user_cost_data = {'active': True, 'cost': float(free_tier_cost_limit) + 1.0, 'developer_id': developer_id, 'tags': []}
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.get('/test-get-with-cost-limit', headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_200_OK
...
def test_middleware_cost_is_none_treats_as_exceeded_limit(client):
    ...
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.post('/test-none-cost', json={'message': 'test'}, headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_middleware_null_tags_field_handled_properly(client):
    ...
    async def test_null_tags(payload: TestPayload):
        return {'status': 'success', 'method': 'POST', 'message': payload.message}
    developer_id = str(uuid.uuid4())
    mock_user_cost_data = {'active': True, 'cost': float(free_tier_cost_limit) + 5.0, 'developer_id': developer_id, 'tags': None}
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.post('/test-null-tags', json={'message': 'test'}, headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert 'Cost limit exceeded' in response.text
        assert 'cost_limit_exceeded' in response.text
...
def test_middleware_forbidden_if_user_is_not_found(client):
    ...
    developer_id = str(uuid.uuid4())
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(side_effect=asyncpg.NoDataFoundError())):
...
def test_middleware_valid_user_passes_through(client):
    ...
    developer_id = str(uuid.uuid4())
    mock_user_cost_data = {'active': True, 'cost': 0.0, 'developer_id': developer_id, 'tags': []}
    with patch('agents_api.web.get_usage_cost', new=AsyncMock(return_value=mock_user_cost_data)):
        response = client.get('/test-valid-user', headers={'X-Developer-Id': developer_id})
        assert response.status_code == status.HTTP_200_OK
        assert response.json()['message'] == 'valid user'

async def test_middleware_cant_create_session_when_cost_limit_is_reached(make_request, pg_dsn, test_agent):
...
async def test_middleware_cant_delete_session_when_cost_limit_is_reached(make_request, pg_dsn, test_session, test_agent):
    ...
    with patch('agents_api.web.get_usage_cost', new=mock_get_usage_cost):
        ...
        assert 'Cost limit exceeded' in delete_response.text
        assert 'cost_limit_exceeded' in delete_response.text
        mock_get_usage_cost.side_effect = [{'active': True, 'cost': float(free_tier_cost_limit) + 1.0, 'developer_id': developer_id, 'tags': []}]
        get_response = make_request(method='GET', url=f'/sessions/{session_id}', headers={'X-Developer-Id': str(developer_id)})
        assert get_response.status_code == status.HTTP_200_OK