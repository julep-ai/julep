...
invalid_task_dict = {
    ...
    "main": [
        {"evaluate": {"result": "$ 1 + )"}},
        {"if": "$ undefined_var == True", "then": {"evaluate": {"value": "$ 'valid'"}}},
    ],
}
valid_task_dict = {
    "name": "Test Task",
    ...
    "main": [
        {"evaluate": {"result": "$ 1 + 2"}},
        {"if": "$ _ is not None", "then": {"evaluate": {"value": "$ str(_)"}}},
    ],
}


@pytest.mark.skip(
    reason="CreateTaskRequest model not fully defined - needs investigation"
)
...
nested_task_with_error_dict = {
    ...
    "main": [
        {
            "if": "$ _ is not None",
            "then": {"evaluate": {"value": "$ undefined_nested_var"}},
            "else": {"if": "$ True", "then": {"evaluate": {"result": "$ 1 + )"}}},
        },
        {
            "match": {
                "case": "$ _.type",
                "cases": [
                    {"case": "$ 'text'", "then": {"evaluate": {"value": "$ 1 / 0"}}}
                ],
            }
        },
        {
            "foreach": {
                "in": "$ range(3)",
                "do": {"evaluate": {"result": "$ unknown_func()"}},
            }
        },
    ],
}
...
def test_recursive_validation_of_if_else_branches():
    ...
    step_with_nested_if = {
        "if": {"if": "$ True", "then": {"evaluate": {"value": "$ 1 + )"}}}
    }
...
def test_recursive_validation_of_match_branches():
    ...
    step_with_nested_match = {
        "match": {
            "case": "$ _.type",
            "cases": [
                {"case": "$ 'text'", "then": {"evaluate": {"value": "$ undefined_var"}}}
            ],
        }
    }
    ...
    for issue in validation_results["main"]["0"]:
        if "undefined_var" in str(issue["expression"]) and "Undefined name" in str(
            ...
            nested_error_found = True
    assert nested_error_found, (
        "Did not detect undefined variable in nested case structure"
    )
...
def test_recursive_validation_of_foreach_blocks():
    ...
    step_with_nested_foreach = {
        "foreach": {
            "in": "$ range(3)",
            "do": {"evaluate": {"value": "$ unknown_func()"}},
        }
    }
    ...
    for issue in validation_results["main"]["0"]:
        if "unknown_func()" in str(issue["expression"]) and "Undefined name" in str(
            ...
            nested_error_found = True
    assert nested_error_found, (
        "Did not detect undefined function in nested foreach structure"
    )
