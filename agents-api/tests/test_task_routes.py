...
from agents_api.autogen.openapi_model import ExecutionStatusEvent, Transition
...
def test_route_unauthorized_should_fail(client, test_agent):
    ...
    data = {'name': 'test user', 'main': [{'kind_': 'evaluate', 'evaluate': {'additionalProp1': 'value1'}}]}
...
async def test_route_create_task_execution(make_request, test_task):
    ...
    async with patch_testing_temporal():
        response = make_request(method='POST', url=f'/tasks/{test_task.id!s}/executions', json=data)
    assert response.status_code == 201
...
async def test_route_list_all_execution_transition(make_request, test_execution_started):
    response = make_request(method='GET', url=f'/executions/{test_execution_started.id!s}/transitions')
    assert response.status_code == 200
...
async def test_route_list_a_single_execution_transition(pg_dsn, make_request, test_execution_started, test_developer_id):
    pool = await create_db_pool(dsn=pg_dsn)
    scope_id = uuid7()
    transition = await create_execution_transition(developer_id=test_developer_id, execution_id=test_execution_started.id, data=CreateTransitionRequest(type='step', output={}, current={'workflow': 'main', 'step': 0, 'scope_id': scope_id}, next={'workflow': 'wf1', 'step': 1, 'scope_id': scope_id}), connection_pool=pool)
    response = make_request(method='GET', url=f'/executions/{test_execution_started.id!s}/transitions/{transition.id!s}')
    assert response.status_code == 200
    response = response.json()
...
def test_route_stream_execution_status_sse_endpoint(client: TestClient, test_execution_started, test_developer_id):
    ...
    with patch('agents_api.routers.tasks.stream_execution_status.execution_status_publisher', mock_sse_publisher), client.stream('GET', url, headers=headers) as response:
        ...
        for i, line in enumerate(response.iter_lines()):
            ...
            if len(received_events) >= len(mock_sse_responses) or i >= max_attempts:
                break
        response.close()
    assert len(received_events) == len(mock_sse_responses), f'Expected {len(mock_sse_responses)} events, got {len(received_events)}'
...
def test_route_stream_execution_status_sse_endpoint_non_existing_execution(client: TestClient, test_developer_id):
