...
def get_workflow_name(transition: Transition) -> str:
    ...
    if workflow_str.startswith("PAR:`"):
        ...
        assert len(workflow_str) > 5 and "`" in workflow_str[5:], "Workflow string is too short or missing backtick"
        workflow_str = workflow_str[5:].split("`")[0]
    elif workflow_str.startswith("`"):
        # Extract between backticks
        assert len(workflow_str) > 1 and "`" in workflow_str[1:], "Workflow string is too short or missing backtick"
