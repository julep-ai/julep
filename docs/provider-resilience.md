...
LLM providers go down. The framework's answer is a worker-side resilience
layer at the `LlmCaller` seam: a **deterministic fallback chain** per model, an
...
(`composable_agents.resilience`); the mechanism is
`make_resilient_llm_caller` in `composable_agents.execution.llm`.
