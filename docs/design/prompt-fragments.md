...
It references [SPEC.md](../SPEC.md) (§6.4 pure-function drift, §8.2 WHOLE_SESSION
degradation, §10 agent loop), [algebra.hs](./algebra.hs) (the `project`/`decide`
model), and [typed-flow.md](./typed-flow.md) (freeze-isolation: every callable is
...
- **Drift detection like a pure.** The renderer's *body* is not in the artifact;
  ...
  ([SPEC §6.4 pure-function drift](../SPEC.md#64-pure-function-drift)), so a
...
[agent-loop-as-turn.md](./agent-loop-as-turn.md): a **monoid homomorphism**
...
[SPEC §8.2](../SPEC.md#82-whole_session-degradation)), not inline.
...
- **Determinism / freeze isolation ([typed-flow.md §2](./typed-flow.md)).** A
