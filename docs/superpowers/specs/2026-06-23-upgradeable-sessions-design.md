...
**Budget accounting.** Session budget is enforced at **turn granularity**. The breaching turn completes, its emitted records are delivered, and then the workflow appends `Closed(over_budget)`; a single turn may overshoot. Non-fatal-error turns are still charged for segment spend and can trip the same `over_budget` close before the next receive.
...
`emit` does **not** call out over the network (so it cannot be retried into a duplicate, Codex MAJOR #4). It **appends to a durable, seq'd output-channel buffer in workflow state and records the emitted value in the durable event log**; external consumers (`SessionHandle.events()`) consume the event log as the single source of emitted records. Therefore `emit` is deterministic and replay-safe by construction. `recv`/`emit` carry explicit **non-retryable** contracts and do **not** inherit the call-retry loop (`interpreter.py:320`) nor any activity `RetryPolicy` (`harness.py:269`). The transport from the event log to a remote consumer is at-least-once; consumers dedup by `(channel, seq)`, making it **effectively-once**.

**Output-buffer retention (Codex 3rd-pass).** The output buffer is bounded and drained by the event-log ack: `SessionHandle.events()` acks the highest delivered event sequence, and the workflow advances the corresponding output-channel ack cursors and evicts every output item represented by acked emit events. Steady-state buffer size is bounded by in-flight (un-acked) events, not by session lifetime or continue-as-new carry. If the buffer fills (slow/absent consumer), `emit` **parks on `wait_condition` until space frees** — durable and replay-safe, symmetric with input backpressure (§7.5) — and the high-watermark is surfaced via `state()` so a stuck/unconsumed session is observable rather than silently growing history. v1 is **single-consumer per handle on both local and Temporal**; multi-consumer fan-out (which complicates the ack cursor) is deferred (§14.5).
...
- `close()` → `Closed`; on Temporal it awaits quiescence before returning (matching local), and on the CMA backend it maps to `CMASession.cancel()`.
- A CMA `terminal` is adapted to `Turn("done")`, a CMA `error` to `Error(fatal=…)`.
- A fatal Temporal turn appends durable `Error(fatal=True)` followed by durable `Closed(reason)` before the workflow result fails, so `Closed` remains the only public terminator.
...
**M1.5 — carrier + safety — DONE on `main` (`43182b7`,`bd9dbda`; 1342 passed, mypy clean).**
- **Carrier threading (BLOCKER) — resolved.** `recv` had discarded the carrier (state threaded only via an external closure). Resolution: `recv` stays a body leaf (so the §7.1 recv-guard stays meaningful), **`emit` is carrier-transparent** (passes its input through; the payload goes to `env.emit`), and the turn carries `(carrier', output)` explicitly — `scan(step, init)` marks the `LOOP` `split` so the driver splits a 2-tuple; `loop(...)` is the lower form where the carrier *is* the body value and emits are transparent taps. `drive_session` now runs `session.body` through `interpret()` over a channel-backed `InMemoryEnv` — **one** driver shared with the interpreter `Op.LOOP` case. Exactly-one `recv` per path is enforced (`SESSION_LOOP_MULTIPLE_RECV`), giving a well-defined turn boundary without hoisting `recv` into the driver.
- **Flow-vs-session target gate (BLOCKER) — resolved.** `validate(flow, target="flow"|"session"|None)`: `target="flow"` rejects any root `LOOP` (`SESSION_LOOP_IN_FLOW`) and stray `recv`/`emit`; `target="session"` requires a root `LOOP`. Also `SESSION_LOOP_BAD_FIELDS` (LOOP carrying non-body fields) and blocking `SESSION_CHANNEL_UNDECLARED`. *(M2 wires `deploy`/`FlowWorkflow` to pass `target="flow"`; that hard-rejection is M2.)*
...
**M2-facade — `Agent.open` surface — DONE on `main` (`04916c5`,`df90161`; 1361 passed, mypy clean).** `Agent.open(*, session, backend="local"|"temporal") -> SessionHandle` (sibling of `arun`/`deploy`); `SessionEvent` (`Emit`/`Turn`/`Error`/`Closed`, ends only on `Closed`); `LocalSessionHandle` (in-memory channel-backed live driver) + `TemporalSessionHandle` (client-side: `send`/`ack`/`close` Updates, `state`/`open_receives` queries, **yield-then-lazy-ack** so a consumer crash can't drop events); `open_receives()` on the Protocol; per-channel idempotency dedup; `deploy`/freeze pass `target="flow"` (ordinary flows hard-reject `LOOP`/`recv`/`emit`); orphan `PLANNED` span suppressed; runtime-compiled plans re-validated. `open_session()` (sync) raises for local (dead-loop footgun) — use `await open()`.
  - *M2-hardening closed:* **(a)** guard wiring parity now runs through session deployment for Temporal and local; Temporal budget rejects unsupported `tokens`/`wallSeconds` loudly and still enforces `cost`; `maxCalls` counts merge back from child `Op.APP` agents across session turns; **(b)** Temporal event logs carry uniform `Turn` markers; **(c)** non-fatal Temporal turns append `Error(fatal=False)` + `Turn(done)`, run budget/CAN bookkeeping, and continue; **(d)** Temporal fatal turns append durable `Error(fatal=True)` + `Closed(reason)`; **(e)** Temporal `close()` awaits quiescence and preserves the in-flight final emit; **(f)** output-buffer eviction is driven by event-log ack and `events()` is single-consumer per handle on both local and Temporal.
  - *API tightening:* public `start_session` is now internal (`_start_session`) and assumes a validated Deployment produced by `Agent._deploy_session`.

**M3 — facade + CMA — DONE on `main`.** CMA backend for `SessionHandle` (one-session-per-turn, unifies `CMASession`), now reachable via `Agent.open(session=…, backend="cma", client=…)` and routing **all per-turn custom-tool servicing through the same enforcement plane as `drive_cma_agent_loop`** (deny-by-default `authorize_call` + contracts, `charge_tool_call`/`maxCalls`, `would_exceed_budget`, `max_rounds`, `precheck_controller`, strict-vs-dev denial; per-turn `AgentState` threads `state.last` so input-less tool events reuse the prior value). CMA `send` honors `idempotency_key` (dedup by `(channel, key)`→`(seq, fingerprint)`, raise on same-key/different-payload), rejects unsupported channels, and `close()` is bounded (cancels the driver + `wait_for` timeout, always emits exactly one `Closed`). `state()` shares the local backend's common keys (`closed`/`capacity`/`pending`) plus an additive `turns`. `@session` coroutine sugar (carrier lifting) compiles to `Op.LOOP`: the generated step pure is registered under a **content-addressed name+hash derived from the canonical `ast.unparse` of the step** (not `inspect.getsource` of the exec'd fn, which read the wrong file region and collided across distinct sessions), re-registration of identical canonical source is idempotent, and carrier lifting now lifts **every** pre-loop local referenced (read *or* write, incl. `AugAssign`) after the `recv` boundary. `ca chat` opens a local `SessionHandle`; `ca listen`/`ca trigger` shipped in M3 as **thin local-session wrappers** (pulled forward from the deferred bucket — they are pure wrappers over the same local handle, add no reactive machinery). `ca trigger --channel` validates the channel against the session's input channel up front (errors instead of hanging on an unbound channel); `ca chat`/`ca listen` read stdin off the event loop so the event consumer streams concurrently. `ca chat`'s local wrapper uses `loop(...)` (carrier-is-body-value, transparent emits), not `scan(split=True)`.

> **Turn-marker semantics across backends.** `Turn(started)` is emitted **per accepted inbound turn** on the CMA backend (one CMA session per inbound message); the local backend emits it recv-gated (when a `recv` actually consumes input). For the supported `recv`→work→`emit` turn shapes these coincide.
