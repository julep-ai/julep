...
**Decisions (locked in brainstorming):** freeze+publish+ledger (no infra rollout) · committed lockfile `.ca/deploys/<env>.json` · this increment = deploy + status + `run --env` · freeze mirrors `_cmd_freeze` (`strict=False`, no caps manifest) · drift = re-freeze + compare `artifact_hash`.
...
```
composable_agents/ca/config.py        EXTEND: EnvConfig dataclass + CaConfig.envs (parse [tool.ca.env.<name>]); implicit `local`
composable_agents/ca/ledger.py        NEW: DeployRecord + read_ledger/upsert_record (.ca/deploys/<env>.json)
...
.gitignore                            ADD: .ca/runs/ and .ca/cas/ (ignored); .ca/deploys/ stays tracked
tests/ca/test_envconfig.py test_ledger.py test_deploy.py test_status.py test_temporal_run.py test_deploy_cmd.py
```
...
**Contract:** add `EnvConfig(name, temporal_address: str|None, temporal_namespace: str, task_queue: str, cas: str|None, langfuse_host: str|None)` and `CaConfig.envs: dict[str, EnvConfig]`. Parse `[tool.ca.env.<name>]` tables (pyproject + ca.toml override, same precedence as the rest of config). Always include an implicit `local` env (cas defaults to `.ca/cas`, no temporal). `cas` is a URI: `s3://bucket/prefix` or a local path (default `.ca/cas`).

- [ ] Test (`tests/ca/test_envconfig.py`): defaults give `envs["local"]` with `cas` ending `.ca/cas`, `temporal_address is None`; a `[tool.ca.env.staging]` table parses address/namespace/task_queue/cas; `ca.toml` overrides an env field.
- [ ] Implement; keep existing `CaConfig` fields/behavior unchanged. `ruff` + `mypy --strict` clean.
...
- [ ] Commit `feat(ca): committed deploy ledger (.ca/deploys/<env>.json)`.
...
- [ ] Test (`tests/ca/test_deploy.py`, **local CAS, fully verifiable**): using the `sample_module` fixture + a `[tool.ca.env.local]`, `deploy_agents(cfg, ["triage"], "local", now_iso="2026-06-23T00:00:00Z")` returns a record with a `sha256:`-prefixed `artifact_hash`, writes `.ca/deploys/local.json`, and the bundle lands under `.ca/cas/`. `freeze_agent` on an unknown agent → error surfaced, no ledger write.
...
- [ ] Test (`tests/ca/test_status.py`, local): deploy `triage` to `local`, then status shows `triage` clean; edit triage's source so its hash changes → status shows `drift` and `status_exit_code == 3`; an agent never deployed shows `undeployed`.
- [ ] Implement; `ruff` + `mypy --strict` clean.
...
- [ ] Test (`tests/ca/test_temporal_run.py`): `local` env routes to the in-memory runner and returns events (reuse the Task-5 runner). For the cloud path, inject a **fake client** whose `execute_workflow` records the args and returns a canned result; assert `run_flow` is called with the deployed `flow_json` + `task_queue` from the env. (No live Temporal — integration-pending, stated in the test docstring.)
- [ ] Implement; `ruff` + `mypy --strict` clean.
- [ ] Commit `feat(ca): ca run --env via run_flow against a deployed agent`.

## Task 6 — CLI wiring + .gitignore (`cli.py`)

**Contract:** add `ca deploy <selector> --env <name>` (default `local`; freezes+publishes+records selected agents; prints each agent + short artifact_hash; exit 0, or 1 on any freeze error). `ca status --env <name> [selector]` (prints rows + state; `raise typer.Exit(status_exit_code(rows))`). Extend `ca run` with `--env <name>` (default `local`): local keeps today's behavior; non-local calls `run_on_env`, renders the trace tree from returned events if available, prints the Langfuse deep link. Add `.gitignore` lines `.ca/runs/` and `.ca/cas/` (keep `.ca/deploys/` tracked).

- [ ] Tests (`tests/ca/test_deploy_cmd.py`, local): `ca deploy triage --env local` exit 0 + `.ca/deploys/local.json` written; `ca status --env local` after deploy → exit 0 prints `clean`; `ca status` with a drifted source → exit 3; `ca run triage --env local` still renders a tree (unchanged local path).
- [ ] Implement; full `tests/ca` green; `ruff` + `mypy --strict` clean.
