...
- [ ] TDD → gates → commit `feat(phase4): ca schedule — Temporal Schedules from julep.toml, drift-aware`
...
- Test: `tests/cli/test_queue_lanes.py`, extend `tests/invariants/test_split.py`

**Interfaces:**
- Produces: `[env.prod.queues] foreground = "prod-fg" maintenance = "prod-maint" ...` — an n-lane map per env (julep.toml). Three layers, all omit-when-unset so every existing golden/ledger hash is byte-identical:
...
- mem-mcp mapping note (module docstring + helm): the real role lanes above map to one worker Deployment + KEDA ScaledObject per lane (values example added to `infra/helm/julep-worker/values.yaml` as comments); interactive paths (RECORD plan/execute, checkin/recall) vs background (sweeps, rollups, clustering, dream) is the two-lane starter, not the ceiling.
