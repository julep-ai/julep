...
**Architecture:** Phase 3 changes concentrate in the pure loop core (`agent_loop.py`, `turn.py`, `transcript.py`), the provider seam (`execution/llm.py`, `execution/llm_result.py`), and the facade (`agent.py`); every new loop behavior is a pure, JSON-serializable state/config change so all four backends (local/DBOS/Temporal/CMA) inherit it. Native tool-calling is a passthrough: any-llm's `acompletion` already accepts `tools`/`tool_choice`/`parallel_tool_calls` and returns `message.tool_calls`; we add the wiring, the decision vocabulary, and the transcript round-trip. Phase 4 sits in `execution/llm.py` (cache_control content blocks), `trajectory.py` (redaction seam), `execution/worker.py` + a new replay-corpus test harness (versioning), and `ca` (schedules + queue lanes).
...
**Files:**
- Modify: `composable_agents/dotctx.py` (`Reasoner.prompt_cache: Optional[str] = None`, settings key `prompt_cache` accepting `"5m"|"1h"`, teaching error otherwise), `composable_agents/dotctx_rich.py` (`_ALLOWED_SETTINGS`), `composable_agents/deploy.py:_reasoner_identity` (`"promptCache"` omit-when-unset), `composable_agents/execution/llm.py` (`_messages` + `call` closure), `composable_agents/execution/llm_result.py` (cache usage fields)
- Test: `tests/test_llm_prompt_cache.py`

**Interfaces:**
- Produces: for `provider == "anthropic"` and `prompt_cache` set, the system message is emitted in content-block form — `{"role": "system", "content": [{"type": "text", "text": system_text, "cache_control": {"type": "ephemeral", "ttl": "1h"}}]}` (ttl omitted for `"5m"`, Anthropic's default) — and on transcript-bearing calls the *last* transcript message also carries a `cache_control` block (two markers: stable prefix + growing conversation, the standard agent-loop pattern). `LlmCallMeta.cache_read_tokens` / `cache_creation_tokens: int | None` populated from `usage.prompt_tokens_details`/anthropic-style `cache_read_input_tokens`/`cache_creation_input_tokens` when present (attrs `"llm.cache"` dict, omit-when-absent).

Behavior:
- Non-anthropic providers ignore the field (no content-block rewrite — OpenAI-side caching is automatic); a `prompt_cache` value on a non-anthropic model is allowed and inert (documented), so a resilience ladder crossing providers never errors on the field.
- `_messages` keeps returning plain-string content when `prompt_cache` is unset — every existing test and golden is untouched.
- If any-llm rejects list-content messages for anthropic (verify against the pinned version first — spike step below), fall back to threading `extra_body`/provider kwargs per any-llm's anthropic adapter; the test seam asserts on the kwargs actually sent, so the implementation detail stays behind the fake.
- Verification spike (step 1 of the task, before the TDD cycle): one live anthropic call through any-llm with a content-block system message + `cache_control`, assert `cache_creation_input_tokens > 0` on first call and `cache_read_input_tokens > 0` on second (keys from `.env`; mark `@pytest.mark.live`, excluded from CI).

Tests: anthropic + `prompt_cache="1h"` → block-form system with ttl, last transcript turn marked; `"5m"` → no ttl key; unset → plain strings (golden equality); openai + set → plain strings; settings loader accepts/validates values, identity omits-when-unset (deploy goldens unchanged); meta cache fields from a fake usage payload; `to_attrs` omission rules.

- [ ] Spike (live, keys from `.env`) → TDD → gates → commit `feat(phase4): anthropic prompt_cache — cache_control markers + cache usage meta`

### Task 10: Trajectory redaction (PII/secrets never land raw)

**Files:**
- Modify: `composable_agents/trajectory.py` (recorder write path + export), reuse the secret-key heuristic from `composable_agents/validate.py:61` (extract to a shared helper if it isn't importable cleanly)
- Test: `tests/test_trajectory_redaction.py`

**Interfaces:**
- Produces: `Redactor = Callable[[Any], Any]`; `TrajectoryRecorder(..., redactor: Optional[Redactor] = None)` — when None, the default `redact_secret_shaped(value)` applies: any dict key matching the validate.py secret-key pattern (`key/token/secret/password/authorization/api_key`-shaped) has its value replaced by `"[REDACTED]"`, recursively. `export_trajectory_jsonl_hydrated(..., redactor: Optional[Redactor] = None, allow_raw: bool = False)` — hydrated values pass through the redactor; `allow_raw=True` is the only way to skip it, and it logs a warning naming the export path.

Behavior:
- Redaction happens at the *record* seam (before canonical-JSON/blob write) for tool inputs/outputs and reasoner payloads — the trajectory store never contains the raw secret, so no later export path can leak it. Projection/observability planes are NOT touched (they are ref-based and per-run; scope creep here breaks the Langfuse attr snapshots).
- The default heuristic is deliberately conservative (keys only, not value-shaped detection); hosts with real PII obligations (mem-mcp: memory content) inject their own redactor — the seam is the deliverable, the default is the floor. Document the mem-mcp expectation (hash-pointer redactor) in the module docstring.
...
Tests: tool input `{"api_key": "sk-...", "q": "hi"}` recorded → stored value has `[REDACTED]` + intact `q`; nested dicts/lists recurse; custom redactor replaces default; export without `allow_raw` applies redactor to hydrated blobs; `allow_raw=True` warns; raising redactor drops the record and increments the failure counter (nothing raw persisted).

- [ ] TDD → gates → commit `feat(phase4): trajectory redaction seam — secret-shaped scrub by default, fail-closed`

### Task 11: Temporal replay-versioning discipline

**Files:**
- Modify: `composable_agents/execution/worker.py` (Build-ID wiring), `CONTRIBUTING.md` (policy section)
...
**Interfaces:**
- Produces: `start_worker(..., build_id: Optional[str] = None, use_worker_versioning: bool = False)` — defaults preserve today's behavior; when `use_worker_versioning` is set, `build_id` defaults to the package version (`importlib.metadata.version("composable-agents")`) and is passed to `temporalio.worker.Worker(build_id=..., use_worker_versioning=True)`. `serve.py` reads `CA_WORKER_BUILD_ID` / `CA_WORKER_VERSIONING=1`.

Behavior:
- **The corpus is the gate.** `record_histories.py` runs the existing time-skipping e2e scenarios (flow with par/each/sub, agent loop with continue-as-new, session with store) against the *current* code and saves `WorkflowHistory` JSON per scenario. `test_replay_corpus.py` replays every stored history through `temporalio.worker.Replayer(workflows=[FlowWorkflow, AgentWorkflow, SessionWorkflow])` — any nondeterministic change to the interpreter/harness (cid allocation, par scheduling, new commands) fails this test *before* it ships, which is exactly the failure mode M8 identified (only 3 `workflow.patched()` gates exist today, `harness.py:283-300`, all bundle plumbing).
...
Tests: corpus replays clean at HEAD; a deliberate scheduling mutation (test-local monkeypatch reordering a gather) makes the replayer raise (proves the gate has teeth); `start_worker` passes build_id/versioning kwargs through (fake Worker capture); defaults byte-identical to today.
...
Behavior:
- `apply` builds `temporalio.client.Schedule(action=ScheduleActionStartWorkflow(FlowWorkflow, args=[frozen artifact input...], task_queue=env.task_queue), spec=ScheduleSpec(cron_expressions=[cron]))` with `schedule_id = f"ca:{env}:{name}"`; overlap policy SKIP (one running instance per schedule — matches mem-mcp's APScheduler + self-dedup posture); paused honored.
...
Tests: toml parse matrix (defaults, bad cron string → teaching error at parse via `croniter`-free regex sanity check: five/six fields), undeployed-flow refusal, schedule_id shape, apply/ls/rm against a fake client capturing calls; `@skipif` integration: apply → server schedule exists → rm → gone.
...
**Files:**
- Modify: `composable_agents/ca/config.py` (`EnvConfig.queues: dict[str, str]`), `composable_agents/ca/cli.py` (`ca run --queue`, `serve`/worker selection), `composable_agents/deploy.py` (per-agent `queue` deploy metadata), `composable_agents/execution/worker.py` (no change needed if task_queue already parametrized — verify)
- Test: `tests/ca/test_queue_lanes.py`

**Interfaces:**
- Produces: `[env.prod.queues] interactive = "prod-interactive" background = "prod-background"` (ca.toml); flows/agents opt into a lane via `deploy(..., queue="interactive")` recorded in the ledger (omit-when-unset → default `env.task_queue`, ledger hashes for unset flows byte-identical); `ca run --env prod <flow>` starts the workflow on the flow's resolved lane; worker entrypoint gains `--queue <lane-name|raw>` (resolves lane through config, falls back to raw string).

Behavior:
- Lane resolution is config-time only — the workflow itself never knows its lane (no IR/determinism impact). Child workflows/activities inherit the parent's task queue (today's behavior) so a lane is a whole-pipeline property; document that mixed-lane pipelines are out of scope (YAGNI until a real need).
- mem-mcp mapping note: interactive lane = RECORD plan/execute + checkin/recall paths; background lane = sweeps, rollups, clustering, dream — two worker Deployments in helm, each with its own KEDA ScaledObject (values example added to `infra/helm/ca-worker/values.yaml` as comments).

Tests: config parse + default lane fallback; deploy metadata omit-when-unset (golden equality) and present-when-set; `ca run` resolves lane → start call captures expected task_queue (fake client); worker `--queue` lane-name and raw-string resolution; unknown lane name → teaching error listing configured lanes.

- [ ] TDD → gates → commit `feat(phase4): queue lanes — per-env named task queues, per-deployment lane selection`
...
**Phase 4:** anthropic calls with `prompt_cache="1h"` emit cache_control (live-spike verified) and record cache-read usage; a secret-shaped tool input never reaches the trajectory store raw; the replay corpus gates harness changes in CI; `ca schedule apply` + two-lane workers demonstrated against the dev Temporal (guarded integration tests).
...
- Mixed-lane pipelines, per-tenant credential resolution, Temporal server multi-env Terraform productionization (infra track, not framework).
