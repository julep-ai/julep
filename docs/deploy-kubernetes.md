# Deploy on Kubernetes with KEDA autoscaling
...
a Deployment of worker pods, autoscaled by [KEDA's `temporal`
scaler](https://keda.sh/docs/latest/scalers/temporal/) (KEDA ≥ 2.17) on
task-queue backlog, including scale-to-zero for background lanes.

The framework side is `composable_agents.execution.serve`: an env-configured
entrypoint (`composable-agents worker`) with SIGTERM graceful drain and
`/healthz` + `/readyz` probes. For what a worker is, see
...
- **Provider circuit breakers are per-process**
  ([provider-resilience.md](provider-resilience.md)): each replica discovers a
...
Related: [deploy-temporal.md](deploy-temporal.md),
[dispatch-boundary.md](dispatch-boundary.md),
[provider-resilience.md](provider-resilience.md), [docs index](README.md).
