...
[Team Guide](team-guide.md); for the why see [Concepts](concepts.md); for the
normative contract see [SPEC](SPEC.md).
...
source hashes). [Determinism contract →](AUTHORING.md#determinism-contract)
...
- Omit `llm` → keyless default reasoner: emits one `RuntimeWarning` and returns
  input unprocessed. ([why](gotchas.md#keyless-default-reasoner))
...
[Gotchas](gotchas.md) · [Authoring Guide](AUTHORING.md#define-time-errors).
