...
:mod:`julep.execution`; the class here is the seam). Everything in
...
in :mod:`julep.execution.otel` consumes.
"""
...
class PostgresProjection:
    ...
    wiring lives in :mod:`julep.execution` so this module stays
    ...
    def append(self, event: ProjectionEvent) -> None:
        if self._execute is None:
            raise RuntimeError(
                ...
                "from julep.execution or use InMemoryProjection in tests"
            )
        row = event.to_json()
