"""Command-line inspection tools for Julep artifacts."""
...
from julep.capabilities import CapabilityManifest, check_approval_gates
from julep.contracts import McpAnnotations, ToolContract, ToolManifest, manifest_from_json
from julep.deploy import Deployment, deploy, snapshot_from_listings
...
from julep.ir import CallStep, Node, SubStep, pure_display, toolref_key
...
from julep.validate import Diagnostic, blocking, validate
...
def main(
    ...
    *,
    ...
    except (JulepError, KeyError, ValueError, OSError, json.JSONDecodeError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1


def _parser(*, prog: str = "julep") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=prog,
        description="Validate, freeze, inspect, locally run, and graph Julep "
    ...
    worker_p.add_argument(
        "--env", default="local", help="Julep env for --queue lane resolution"
    )
    worker_p.add_argument(
