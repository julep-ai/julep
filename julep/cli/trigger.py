...
from julep.cli.chat import render_event
from julep.cli.config import load_config
from julep.cli.resolve import ResolvedAgent, resolve_agent
from julep.cli.session_local import IN_CHANNEL, open_local_session, parse_json_or_raw
...
def trigger_command(
    ...
    if channel != IN_CHANNEL:
        typer.echo(
            f"error: unsupported channel {channel!r}; julep trigger accepts only {IN_CHANNEL!r}",
            err=True,
        )
        raise typer.Exit(2)
