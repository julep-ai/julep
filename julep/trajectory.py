...
The projection plane in :mod:`julep.projection` is the per-run
...
:mod:`julep.execution.effects`) keyed by ``tenant == root_run_id``;
...
:mod:`julep.execution.trajectory_sql`; this module stays pure
...
logger = logging.getLogger("julep.trajectory")
...
class InMemoryTrajectoryStore:
    ...
    async def export_trajectory_jsonl_hydrated(
        ...
        *,
        ...
        from julep.execution.blobstore import parse_ref
    ...
    def _require_execute(self) -> None:
        if self._execute is None:
            raise RuntimeError(
                ...
                "julep.execution or use InMemoryTrajectoryStore in tests"
            )
...
class TrajectoryRecorder:
    ...
    :func:`julep.execution.effects._capture_effect`; this is the
