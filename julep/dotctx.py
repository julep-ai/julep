...
:mod:`julep.execution.activities` reads them at run time. Context is
...
# (which requires the ``julep[dotctx]`` extra). One loader, one
...
def load_dotctx(path: str, *, env: Optional[Mapping[str, str]] = None) -> Reasoner:
    ...
    ``tools.pyi``) are loaded by :mod:`julep.dotctx_rich`.
