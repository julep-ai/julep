...
Mirrors :mod:`julep.execution.harness` for DBOS. One workflow
...
worker configured via :func:`julep.execution.effects.configure`
...
def decode_policy_error(out: Any) -> Any:
    if isinstance(out, dict) and _POLICY_ERROR_KEY in out:
        ...
        exc_type = _POLICY_ERROR_TYPES.get(payload.get("type"), JulepError)
...
# --------------------------------------------------------------------------- #
class DbosEnv:
    """An :class:`~julep.execution.interpreter.Env` whose effects
...
async def agent_workflow(inp: dict) -> Any:
    ...
    :class:`~julep.execution.harness.AgentInput`, minus the
    ...
    When :func:`~julep.agent_loop.should_continue_as_new` fires
