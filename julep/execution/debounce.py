...
:class:`~julep.execution.harness.FlowWorkflow` run whose input is
the collected list, in arrival order. The target flow just sees ``list[item]``
— typically fanning out with :func:`julep.dsl.each`.
...
async def submit_debounced(
    ...
    *,
    ...
    :func:`julep.freeze.freeze` (then serialized), exactly as for
    :func:`~julep.execution.harness.run_flow`. Configuration is
