...
class SessionValidationError(JulepError):
    """Raised when a public session constructor receives an unsafe LOOP body."""


class SessionCompileError(JulepError):
    ...
    async def _drive(self) -> None:
        ...
        try:
            if turn_body is None:
                raise JulepError("session LOOP missing body")
    ...
    async def send(
        ...
        *,
        ...
        if capacity is not None and self._env.pending_count(ch) >= capacity:
            raise JulepError(f"ChannelFull: channel {ch!r} is full")
