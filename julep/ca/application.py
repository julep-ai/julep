...
from julep.ca.config import CaConfig, EnvConfig
...
def load_application(cfg: CaConfig) -> Application:
    if cfg.application is None:
        raise ValueError(
            "no application configured; set [tool.ca] application = 'your.module:application'"
        )
...
def compile_application(cfg: CaConfig, env: EnvConfig) -> CompiledApplication:
...
def _resolve_deployment_config(
    ...
    *,
    ...
    if any(re.fullmatch(r"[0-9a-f]{64}", value) is None for value in allowed_signers):
        raise ValueError(
            "CA_BUNDLE_ALLOWED_SIGNERS must contain comma-separated "
            "64-character hexadecimal Ed25519 public keys"
        )
    signing_key = os.environ.get("CA_BUNDLE_SIGNING_KEY")
    if require_private_signer:
        signer = bundle_signer_public_key(signing_key)
        if allowed_signers and signer not in allowed_signers:
            raise ValueError(
                "the CA_BUNDLE_SIGNING_KEY public key is not present in the "
                "configured CA_BUNDLE_ALLOWED_SIGNERS"
            )
        ...
        if signing_key:
            ...
            raise ValueError(
                ...
                "CA_BUNDLE_ALLOWED_SIGNERS worker environment setting"
            )
    worker_environment = {
        **env.worker_environment,
        "STORE_URL": env.release_store,
        "CA_BUNDLE_ALLOWED_SIGNERS": ",".join(allowed_signers),
    }
...
def observe_application(cfg: CaConfig, env: EnvConfig) -> ObservedApplicationState:
