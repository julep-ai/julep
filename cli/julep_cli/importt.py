...
from .utils import add_entity_to_lock_file, get_entity_from_lock_file, get_julep_client, get_lock_file, update_existing_entity_in_lock_file, update_yaml_for_existing_entity
import yaml
...
def agent(
    ...
    output: Path = typer.Option(
        Path.cwd() /  "src/agents", "--output", "-o", help="Path to save the imported agent. Defaults to cwd/src/agents"),
    ...
    if locked_agent := get_entity_from_lock_file(type="agent", id=id):
        ...
        confirm = typer.confirm(f"Do you want to overwrite the existing agent in the lock file and {locked_agent.get('path')}?")
        ...
        # User wants to overwrite the existing agent
            ...
            update_existing_entity_in_lock_file(type="agent", new_entity={
            ...
            typer.echo(f"Agent '{id}' imported successfully to '{
                       agent_yaml_path}'")
    ...
    # Importing a new agent (doesn't exist in lock file)
        ...
        if not yes:
            confirm = typer.confirm(f"Are you sure you want to import agent '{
                                    id}' to '{output}'?")
            if not confirm:
                typer.echo("Operation cancelled")
                raise typer.Exit()
        ...
        try:
            ...
            add_entity_to_lock_file(type="agent", new_entity={
