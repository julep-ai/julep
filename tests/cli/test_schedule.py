...
from julep.cli.config import JulepConfig, EnvConfig, ScheduleConfig, load_config
from julep.cli.ledger import DeployRecord, upsert_records
from julep.cli.schedule import (
    apply_schedules,
    build_schedule,
...
from julep.cli.temporal_run import build_flow_start_args, run_on_env
...
def _cloud_cfg(tmp_path: Path) -> JulepConfig:
    return JulepConfig(root=tmp_path, envs={"staging": _cloud_env(tmp_path)})
...
def test_missing_cron_or_flow_teaching_error(tmp_path: Path) -> None:
    (tmp_path / "julep.toml").write_text('[schedule.x]\nflow = "f"\n', encoding="utf-8")
...
def test_schedule_id_shape() -> None:
    assert schedule_id("prod", "hourly") == "julep:prod:hourly"
...
def test_schedule_drift_matrix() -> None:
    ...
    server = {
        "julep:staging:aaa": {"cron": "0 * * * *", "paused": False},
        "julep:staging:bbb": {"cron": "45 * * * *", "paused": False},
        "julep:staging:orphaned": {"cron": "0 0 * * *", "paused": False},
        "julep:prod:ignored": {"cron": "0 0 * * *", "paused": False},
    }
...
def test_build_flow_start_args_primitives(tmp_path: Path) -> None:
    ...
    assert sa.input == {"k": "v"}
    assert sa.session_id == f"julep:{env.name}:{record.agent}"
...
def test_apply_creates_then_updates_via_fake_client(tmp_path: Path) -> None:
    ...
    assert sid == "julep:staging:hourly-rollup"
    ...
    assert obj.state.note == "julep-managed cron=0 * * * *"
    ...
    assert fake.handles == ["julep:staging:hourly-rollup"]
...
def test_fetch_server_schedules_fake_client() -> None:
    ...
    class _FakeClient:
        ...
        async def list_schedules(self) -> Any:
            self.listed += 1
            return _AsyncIter([_Row("julep:staging:hourly", _Sched())])

    fake = _FakeClient()
    server = fetch_server_schedules(EnvConfig(name="staging"), client=fake)

    assert fake.listed == 1
    assert server == {"julep:staging:hourly": {"cron": "0 * * * *", "paused": True}}
...
def test_drift_in_sync_when_server_cron_normalized_to_calendars() -> None:
    ...
    class _FakeClient:
        async def list_schedules(self) -> Any:
            return _AsyncIter([_Row("julep:staging:hourly-rollup", _Sched())])

    server = fetch_server_schedules(EnvConfig(name="staging"), client=_FakeClient())
    assert server == {"julep:staging:hourly-rollup": {"cron": "0 * * * *", "paused": False}}
...
def test_remove_schedule_fake_client() -> None:
    ...
    assert ok.requested == ["julep:staging:hourly"]
...
def test_cli_schedule_apply_unknown_env_exits_2(sample_module: Path, capsys: Any, monkeypatch: Any) -> None:
    ...
    code = main(["schedule", "apply", "--env", "nope"])
...
def test_cli_schedule_apply_no_schedules(tmp_path: Path, capsys: Any, monkeypatch: Any) -> None:
    ...
    code = main(["schedule", "apply", "--env", "staging"])
