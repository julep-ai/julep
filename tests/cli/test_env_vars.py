...
def test_env_vars_merge_julep_toml_over_pyproject(tmp_path: Path) -> None:
    """Per-key merge, julep.toml over pyproject — same precedence as scalar fields."""
    (tmp_path / "pyproject.toml").write_text(
        "[tool.julep.env.prod.vars]\n"
    ...
    (tmp_path / "julep.toml").write_text(
        "[env.prod.vars]\n"
        'SUMMARY_MODEL = "from-julep-toml"\n',
        encoding="utf-8",
    )
    cfg = load_config(tmp_path)
    assert cfg.envs["prod"].vars == {
        "SUMMARY_MODEL": "from-julep-toml", "KEEP": "kept"}
...
def test_freeze_payload_carries_env_vars(
    ...
    """`julep deploy --env prod` freezes in a child; the payload must carry the vars."""
    from julep.cli import deploy as deploy_mod
