...
# dict.keys(), so it never KeyErrors under julep run's {"output": value} echo stubs.
PURE_SAMPLE = '''
...
def pure_module(tmp_path: Path) -> Path:
    """A julep project whose only agent uses a @pure (the case the suite never covered)."""
    pkg = tmp_path / "pkg"
    pkg.mkdir()
...
def test_lint_resolves_pures_no_unknown_pure(pure_module: Path) -> None:
    ...
    assert "UNKNOWN_PURE" not in codes, f"pure not resolved by julep lint: {findings}"
...
def test_run_large_input_not_limited_by_argv(pure_module: Path) -> None:
    ...
    # not argv. Regression for julep run serializing the input into one argv string
    # (Codex PR#9, P2 / WS1).
    from julep.cli.config import load_config
    from julep.cli.runner import run_agent_local
...
def test_cli_lint_and_run_end_to_end(pure_module: Path) -> None:
    base = [sys.executable, "-m", "julep.cli.main"]
    lint = subprocess.run(
