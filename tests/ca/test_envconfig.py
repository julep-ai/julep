...
def test_implicit_local_env_present_by_default(tmp_path):
    ...
    # local cas defaults to `.ca/cas` (a local path URI)
    assert local.cas is not None
    assert local.cas.endswith(".ca/cas")
...
def test_existing_config_fields_unchanged(tmp_path):
    """EnvConfig support must not disturb the existing CaConfig fields/behavior."""
    cfg = load_config(tmp_path)
...
def test_env_table_parses_from_pyproject(tmp_path):
    """A [tool.ca.env.<name>] table parses address/namespace/task_queue/cas/langfuse_host."""
    ...
    assert staging.task_queue == "ca-staging"
    assert staging.cas == "s3://my-bucket/ca-prefix"
...
def test_ca_toml_overrides_env_field(tmp_path):
    """ca.toml overrides a single env field, same precedence as the rest of config."""
    (tmp_path / "pyproject.toml").write_text(
        "[tool.ca.env.staging]\n"
        'temporal_address = "temporal.staging:7233"\n'
    ...
    (tmp_path / "ca.toml").write_text(
        "[env.staging]\n"
        'temporal_namespace = "from-ca-toml"\n',
        encoding="utf-8",
    )
    cfg = load_config(tmp_path)
    staging = cfg.envs["staging"]
    # overridden field comes from ca.toml
    assert staging.temporal_namespace == "from-ca-toml"
    # non-overridden field still comes from pyproject
    assert staging.temporal_address == "temporal.staging:7233"


def test_ca_toml_can_override_implicit_local(tmp_path):
