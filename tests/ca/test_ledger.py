"""Tests for the committed deploy ledger (.ca/deploys/<env>.json).

The ledger is self-describing: each DeployRecord embeds the frozen flow_json +
manifest_json so `ca run --env` can replay the deployed artifact with no
...
def test_ledger_path(tmp_path: Path) -> None:
    ...
    assert p == tmp_path / ".ca" / "deploys" / "local.json"
    p2 = ledger_path(str(tmp_path), "staging")
    assert p2 == tmp_path / ".ca" / "deploys" / "staging.json"
...
def test_envs_are_isolated_files(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".ca" / "deploys" / "local.json").is_file()
    assert (tmp_path / ".ca" / "deploys" / "staging.json").is_file()
...
def test_written_json_is_stable_and_sorted(tmp_path: Path) -> None:
    ...
    raw = (tmp_path / ".ca" / "deploys" / "local.json").read_text(encoding="utf-8")
...
def test_write_is_idempotent_byte_for_byte(tmp_path: Path) -> None:
    recs = [_rec("triage", "sha256:aaa"), _rec("escalate", "sha256:bbb")]
    upsert_records(tmp_path, "local", recs)
    first = (tmp_path / ".ca" / "deploys" / "local.json").read_text(encoding="utf-8")
    # writing the same records again must not change the file bytes.
    upsert_records(tmp_path, "local", recs)
    second = (tmp_path / ".ca" / "deploys" / "local.json").read_text(encoding="utf-8")
    assert first == second
