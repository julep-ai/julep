...
from composable_agents import (
    # combinators / leaves
    seq, par, alt, iter_up_to, race, hedge, quorum, call, think, sub, stage,
    app, human_gate, mcp, native, Ann,
    # brains + pures
    Brain, register_brain, pure,
    # capability + lifecycle
    Budget, CapabilityManifest, freeze, deploy, snapshot_from_listings,
    start_flow,
    # enums
    Effect, Idempotency, ContextScope, SummaryPolicy,
)
...
ZONE_FANOUT = 12  # zones in this basin; known at deploy time
swarm = par([plan_one_zone() for _ in range(ZONE_FANOUT)])
...
#    (logistics, irrigation) is idempotent and rides along after approval.
# --------------------------------------------------------------------------- #
commit_after_approval = alt(
...
approve_and_commit = seq(
    ...
    alt(
        ...
        cases={
            ...
            # Under the ceiling: auto-approve, but still emit the same shape so the
            # downstream gate logic is uniform.
            "auto": native("auto_approve"),
        },
    ),
...
#    calls are rate-limited; only the two zone sub-flows are invokable.
# --------------------------------------------------------------------------- #
CAPS_YAML = """
budget:
  usd: 400
...
tools:
  - name: weather/ensemble_gfs       {effect: read, idempotency: native, maxCalls: 3}
  - name: weather/ensemble_ecmwf     {effect: read, idempotency: native, maxCalls: 3}
  - name: weather/regional_downscaled{effect: read, idempotency: native, maxCalls: 3}
  - name: weather/enso_outlook_a     {effect: read, idempotency: native}
  - name: weather/enso_outlook_b     {effect: read, idempotency: native}
  - name: soil/moisture_grid         {effect: read, idempotency: native}
  - name: registry/zones             {effect: read, idempotency: native}
  - name: market/commodity_prices    {effect: read, idempotency: native}
  - name: water/rights_registry      {effect: read, idempotency: native}
  - name: capacity/shared_envelope   {effect: read, idempotency: native}
  - name: optimize_allocation        {effect: read, idempotency: native}
  - name: auto_approve               {effect: read, idempotency: native}
  ...
  - name: water/allocate_contract    {effect: dangerous, idempotency: required, approval: required, maxCalls: 1}
  - name: procure/place_order        {effect: dangerous, idempotency: required, approval: required, maxCalls: 1}
  - name: logistics/book_transport   {effect: write, idempotency: required}
  - name: irrigation/push_schedule   {effect: write, idempotency: native}
...
def build_deployment(snapshot):
    ...
    deployment = deploy(season_plan, snapshot, capabilities=caps)

    blocking = [d for d in deployment.diagnostics if d.blocking]
    if blocking:
