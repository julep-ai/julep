export function transformNamespaces(types, operations) {
    // There can only be 1 file namespace - so if scopes is empty then entity belongs at root level
    const programDecs = {
        types: [],
        operations: [],
        namespaces: {},
    };
    expandModels(programDecs, types);
    expandOperations(programDecs, operations);
    return programDecs;
}
function expandModels(programDecs, types) {
    for (const type of types) {
        const { scope } = type;
        const namespace = getNamespace(programDecs, scope) ?? createNamespace(programDecs, scope);
        namespace.types.push(type);
    }
}
function expandOperations(programDecs, operations) {
    for (const operation of operations) {
        const { scope } = operation;
        const namespace = getNamespace(programDecs, scope) ?? createNamespace(programDecs, scope);
        namespace.operations.push(operation);
    }
}
function getNamespace(programDecs, scope) {
    if (!scope.length)
        return programDecs;
    let namespace = programDecs;
    for (const fragment of scope) {
        if (!namespace)
            return;
        namespace = namespace.namespaces[fragment];
    }
    return namespace;
}
function createNamespace(programDecs, scope) {
    let namespace = programDecs;
    for (const fragment of scope) {
        if (!namespace.namespaces[fragment]) {
            namespace.namespaces[fragment] = {
                namespaces: {},
                types: [],
                operations: [],
            };
        }
        namespace = namespace.namespaces[fragment];
    }
    return namespace;
}
//# sourceMappingURL=transform-namespaces.js.map