...
graph TB
    %% Title
    title[Julep Simple Overview]
    
    %% Components
    LLM[LLM<br/>Language Model]
    AGENT[AGENT<br/>Central Orchestrator<br/>Prompt Management]
    USERS[USERS<br/>User Profiles<br/>Individual Info]
    SESSIONS[SESSIONS<br/>Chat Instances<br/>Multiple Active]
    DOCUMENTS[DOCUMENTS<br/>Information Storage<br/>Retrieval System]
    TASKS[TASKS<br/>GitHub Actions-style<br/>Multi-step Workflows<br/>Recipe Instructions]
    TOOLS[TOOLS<br/>Programmatic Interfaces]
    ...
    %% Connections
    AGENT --> |calls| LLM
    SESSIONS -->|interacts with| AGENT
    USERS --> |Joins| SESSIONS
    AGENT -->|Owns| DOCUMENTS
    AGENT -->|Uses| TOOLS
    ...
    classDef agent fill:#3b82f6,stroke:#2563eb,stroke-width:3px,color:#fff
    classDef users fill:#10b981,stroke:#059669,stroke-width:2px,color:#fff
    classDef sessions fill:#06b6d4,stroke:#0891b2,stroke-width:2px,color:#fff
    classDef documents fill:#8b5cf6,stroke:#7c3aed,stroke-width:2px,color:#fff
    classDef tasks fill:#ef4444,stroke:#dc2626,stroke-width:2px,color:#fff
    classDef tools fill:#f59e0b,stroke:#d97706,stroke-width:2px,color:#fff
    classDef toolExample fill:#fbbf24,stroke:#f59e0b,stroke-width:1px,color:#92400e
    ...
    class TASKS tasks
    class TOOLS tools
    class web_search,api_call,file_read toolExample
```
