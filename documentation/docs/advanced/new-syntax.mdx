...
Another significant change is the removal of Jinja templates (`{{ }}`) in LogSteps and PromptSteps. Instead, we now use Python f-strings with the `$` prefix for dynamic content.

#### Old Syntax (Using Jinja):
```yaml
- prompt:
    - role: user
      content: Write me a article about the topic: {{inputs[0].topic}}
```
...
- prompt:
    - role: user
      content: $ f"Write me a article about the topic: {steps[0].input.topic}"
    - role: user
