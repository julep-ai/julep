...
```
<Note>
In the prompt steps, the we use the `$ f''' '''` to access the python expression within the prompt. Moreover all python expression also need to be enclosed inside that `{ }`.
Anything outside that will be considered as a string.
</Note>
...
Python expressions can be used in various task steps. For a complete list of step types and their syntax, refer to the [Step Types table in the README](https://docs.julep.ai/docs/advanced/types-of-task-types).
