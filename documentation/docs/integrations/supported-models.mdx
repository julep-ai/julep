...
We’ve organized our supported models into the following categories to help you find what you need:

### Premium Models

<Tip>
Looking for top-notch quality? Premium models deliver the best outputs and are perfect for production environments where reliability and accuracy are key.
</Tip>
...
<AccordionGroup>
  <Accordion title="API Keys and Authentication" icon="key" defaultOpen={true}>
    <Warning>
    Remember, API keys are a must for paid models. Make sure your environment variables are set up with the right keys before diving in.
    </Warning>
  </Accordion>
