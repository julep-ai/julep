...
<Tabs>
  ...
  <Tab title="Hybrid Search (HybridDocSearch)">
    ...
    | `metadata_filter` | `object`  | Metadata filter to apply to the search                                                     | `{}`    |
    | `num_search_messages` | `int`    | The number of search messages to use for the search (1-50)                                 | `4`     |
    | `alpha`           | `float`   | Weight between text-based and vector-based search (0=pure text, 1=pure vector)            | `0.7`  |
    ...
    | `k_multiplier`    | `int`     | Controls how many intermediate results to fetch before final scoring                        | `5`     |
  </Tab>
</Tabs>
