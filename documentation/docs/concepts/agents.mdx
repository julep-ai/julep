...
Once created, you can manage your agents through various operations:

<CardGroup cols={2}>
<Card title="Retrieving an Agent" icon="magnifying-glass" href="#retrieving-an-agent">
  Get agent details using ID or metadata filters
</Card>

<Card title="Updating an Agent" icon="pen-to-square" href="#updating-an-agent">
  Modify agent parameters like model, instructions, and tools
</Card>

<Card title="Creating/Updating an Agent" icon="square-plus" href="#creating-and-updating-an-agent">
  Create new agents or update existing ones in a single operation
</Card>

<Card title="Deleting an Agent" icon="trash" href="#deleting-an-agent">
  Remove agents from your workspace
</Card>
</CardGroup>
## Retrieving an Agent
...
You can update any parameter of an existing agent using its ID. 

<Note>
Updating tools or instructions will completely replace the previous values.
</Note>
...
## Creating and Updating an Agent

You can create and update an agent using the `create_or_update` methods. Basically what it does is that it first checks if an agent with the same `agent_id already exists. If it does, it updates the existing agent. If it doesn't, it creates a new one.
...
You can delete an agent using its agent ID.
