...
<CardGroup cols={3}>
    <Card title="Handle All Statuses" icon="check">
        Ensure your application gracefully handles all possible execution statuses, including `failed` and `cancelled`.
    </Card>
    <Card title="Polling Interval" icon="clock">
        Choose an appropriate polling interval to balance responsiveness and API usage.
    </Card>
    <Card title="Logging" icon="file">
        Maintain detailed logs of execution statuses and outputs for auditing and debugging purposes.
    </Card>
</CardGroup>
