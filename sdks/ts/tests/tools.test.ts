...
import { Agents_Agent as Agent, Tools_Tool as Tool, Tools_ToolType } from "../src/api";
...
describe("Tools API", () => {
  ...
  beforeAll(async () => {
    client = setupClient();
    testAgent = await client.agents.create({requestBody: {
      name: "test agent",
  ...
  it("tools.create", async () => {
    const response = await client.agents.createTool({id: testAgent.id, requestBody: mockTool});
  ...
  it("tools.update", async () => {
    const response = await client.agents.updateTool({id: testAgent.id, childId: testTool.id, requestBody: mockToolUpdate});
  ...
  it("tools.list", async () => {
    const response = await client.agents.listTools({id: testAgent.id, offset: 0});
