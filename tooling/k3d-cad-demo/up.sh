...
#   2. k3d cluster with the host artifact-store dir bind-mounted at /artifacts
#   3. a GENERIC worker image (no flow code) built once and imported
#   4. publish the grade-scores bundle into the shared artifact-store dir
...
JULEP_ARTIFACT_STORE_DIR="${JULEP_ARTIFACT_STORE_DIR:-/tmp/julep-cad-artifacts}"
...
# --- 2. k3d cluster with the artifact-store dir bind-mounted at /artifacts -- #
mkdir -p "$JULEP_ARTIFACT_STORE_DIR"
if ! "$K3D" cluster list "$CLUSTER" >/dev/null 2>&1; then
  echo "creating k3d cluster $CLUSTER (volume $JULEP_ARTIFACT_STORE_DIR -> /artifacts) ..."
  "$K3D" cluster create "$CLUSTER" --volume "$JULEP_ARTIFACT_STORE_DIR:/artifacts@all" --wait
fi
...
# --- 4. Publish the bundle into the shared artifact-store dir -------------- #
echo "publishing grade-scores bundle into $JULEP_ARTIFACT_STORE_DIR ..."
JULEP_ARTIFACT_STORE_DIR="$JULEP_ARTIFACT_STORE_DIR" ENV_OUT="$ENV_OUT" uv --project "$REPO_ROOT" run python "$HERE/publish.py"
